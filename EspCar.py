...
# '''
    ...
    '''
        # execute cmd...
        if self.cmd == 'T'    and self.speed < 99: self.speed += 1
        elif self.cmd == 'B':
            if self.speed >0: self.speed = 0	#
            if self.speed >-99: self.speed -= 1
        elif self.cmd == 'R'and self.angle <servoCenter+servoFull: self.angle += 2
        elif self.cmd == 'L'  and self.angle >servoCenter-servoFull: self.angle -= 2
        elif self.cmd == '1':    self.speed = 0
        elif self.cmd == '2':    self.angle = servoCenter
        elif self.cmd == '3': 
            self.speed = 0
            self.angle = servoCenter
        elif self.cmd == '4': self.light.value(0) if self.light.value() else self.light.value(1)
        self.cmd = ' '
        self.update()
        
    def update1(self, throttle, steer):
        '''
    Para acelerometro
    '''
        self.speed = throttle
        ...
        self.servo.angle(servoCenter + steer)
...
def smooth(x, maxX, thr, fondo):
    if abs(x) < thr : return 0
    elif abs(x) > maxX: return sign(x) * fondo
    else:
        ret = fondo * ((abs(x)-thr) / (maxX - thr))**2
    return int(ret) * sign(x)
...
def run():
    ...
    def on_rx():
        global latch, bttn, z0
        data = uart.read()
        
        ''' Acelerómetro '''
        if chr(data[0])=="!" and chr(data[1])=="A" :
            bttn = None
            z = unpack('<f', data[10:14])[0]	# Throttle/Break axis
            if z0 == None: z0 = z
            z -= z0
            y = unpack('<f', data[6:10])[0] # Steer axis
            thr = smooth(z,7,1,100)
            steer = smooth(y,7,1,servoFull)
            car.update1(thr, steer)
            time.sleep(0.1)

        ''' Control Pad '''
        if chr(data[0])=="!" and chr(data[1])=="B" :
            if data.decode()[2]=='5': bttn = "T"
            elif data.decode()[2]=='6': bttn = "B"
            elif data.decode()[2]=='7': bttn = "L"
            elif data.decode()[2]=='8': bttn = "R"
            else:
                bttn = str(data.decode()[2])

            latch[bttn] = int(data.decode()[3])
    ...
    while running:
        ...
        if latch[bttn] : # if pressed, increment
            car.cmd = bttn
            car.update0()
        else:		# if released, decrement
            if bttn != None: car.dec()
#             car.update
...
bttn = '4'
latch = {'T':0, 'B':0, 'R':0, 'L':0, '1':0, '2':0, '3':0, '4':0}
